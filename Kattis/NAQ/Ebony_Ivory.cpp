...
int cost(int n1, int n2, int f1, int f2){
    ...
    if(S[n1] < S[n2]){
        ...
        int dist = S[n2]-S[n1];
        return moves[mvInd][f1][f2][dist] == -1 ? INF : moves[mvInd][f1][f2][dist];
    } else {
        int mvInd = mvMap(n2,n1);
        int dist = S[n1]-S[n2];
        return moves[mvInd][f2][f1][dist] == -1 ? INF : moves[mvInd][f2][f1][dist];
    }
    return INF;
}
