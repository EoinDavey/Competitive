...
M,X,Y=max([(l(set([R(a-x,b-y)for(a,b)in s if(a,b)!=(x,y)])),x,y)for(x,y)in s])
f=lambda x:R(x[0]-X,x[1]-Y)
pt=list(F(lambda x:x,i.chain.from_iterable(i.zip_longest(*[list(x[1])for x in i.groupby(sorted(F(lambda a:a!=(X,Y),s),key=f),key=f)]))))[199]
